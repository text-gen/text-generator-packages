...
let packages = JSON.parse(fs.readFileSync('./community-packages.json', 'utf8'));
let stats = JSON.parse(fs.readFileSync('./community-packages-stats.json', 'utf8'));

let githubToken = process.env.gh_token;

console.log(`Updating stats for ${Object.keys(packages).length} plugins`);
let newStats = {};
for (let plugin of packages) {
	let key = plugin.id;
...
let saveStats = () => fs.writeFileSync('./community-packages-stats.json', JSON.stringify(newStats, null, 2), 'utf8');
